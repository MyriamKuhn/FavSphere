...
try {
  ...
  if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($data->url) && !empty($data->title) && !empty($data->description) && !empty($data->fk_category_id)) {
      ...
      if ($link->addLink()) {
        ...
        echo json_encode(["message" => "Lien ajouté avec succès"]);
      } else {
        // Si l'ajout échoue
        http_response_code(500);  // Code 500 Internal Server Error
        echo json_encode(["message" => "Erreur interne du serveur. Impossible d'ajouter le lien."]);
      }
    } else {
